...
beforeEach(() => {
  ...
  jest.spyOn(RestaurantService, "getRestaurants").mockImplementation(() => sampleData)
...
test("when the delete button is clicked the restaurant row will be removed from the table", () => {
  const {getByText, queryByText } = render(<AdminPage />)
  const firstDeleteBtn = getByText("Delete")
  fireEvent.click(firstDeleteBtn);
  expect(queryByText(/Burger Bar by Fatboy's Concepts/i)).not.toBeInTheDocument();
})
